...
* **Normalization Formula**: $z_i = \frac{x_i - \mu}{\sigma}$ (How we center the data to ignore recording volume differences).
* **Mean Derivative Formula**: $\frac{1}{N-1} \sum |z_{i+1} - z_i|$ (Calculating the "speed" of the signal's direction changes).
* **Spike Ratio Formula**: $\frac{1}{N} \sum I(|z_i| > 1.5)$ (Calculating the percentage of severe electrical excursions).
* **Kurtosis Formula**: The 4th statistical moment $\frac{\mu_4}{\sigma^4} - 3$ (Used as a penalty to detect false-positive "single bad pixel" scans).
* **Zero-Crossing Rate Formula**: Measuring how uniquely chaotic the wave frequency is.
